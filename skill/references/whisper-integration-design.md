...
- Bidder generates RSA modulus N=p·q at bid time, knows φ(N) → derives key instantly via `y = x^(2^T mod φ(N)) mod N`, encrypts the opening with a Poseidon keystream under `key = poseidon('VDF_RSW_KEY_V1', ctx, len(y limbs), |y| limbs)` (`|y| = min(y, N−y)`, `ctx = H(auction, bid)`), posts the ciphertext with the commitment `poseidon(pt)`, discards factors. No integrity tag by design — the commitment binds the plaintext.
...
- Wesolowski proof lets a verifier check the squarings happened in O(log T) group ops: `π^L · x^(2^T mod L) == ±|y| (mod N)` in the signed group `Z_N^*/{±1}`, with a 251-bit prime challenge `L` hash-to-prime-derived from `(N, x, |y|, T, nonce)` and 20 Miller–Rabin rounds **inside the verifier program** (an input L, and a composite hashed L, were both demonstrated forgeable in this repo — see SKILL.md pitfalls). The executable also decrypts in-program and outputs `[instance_hash, ctx, poseidon(ct), poseidon(pt)]` for the contract to match against the stored puzzle, ciphertext and commitment.
- **The bidder holds the trapdoor**: knowing φ(N), they can prove ANY output for their own puzzle. Two different proven outputs for one instance are evidence of bidder misbehaviour (void and slash); never accept the first proof as final.
